...
use crate::sqlite::sql::node_text as node_text_sql;

pub(crate) struct SqliteNodeTextRepo {
    pool: SqlitePool,
}

impl SqliteNodeTextRepo {
...
impl NodeTextRepository for SqliteNodeTextRepo {
    async fn list_by_doc(&self, doc_id: DocumentId) -> AppResult<Vec<NodeText>> {
        ...
        let records = sqlx::query_as::<_, NodeTextRecord>(node_text_sql::LIST_BY_DOC)
    ...
    async fn get(&self, node_id: NodeId) -> AppResult<Option<NodeText>> {
        ...
        let record = sqlx::query_as::<_, NodeTextRecord>(node_text_sql::GET)
    ...
    async fn save(&self, node_text: &NodeText) -> AppResult<()> {
        ...
        sqlx::query(node_text_sql::UPSERT)
        .bind(params.node_id)
        .bind(params.text)
    ...
    async fn delete(&self, node_id: NodeId) -> AppResult<()> {
        ...
        sqlx::query(node_text_sql::DELETE)
            .bind(uuid_to_blob(node_id.as_uuid()))
            .execute(self.pool.pool())
            .await
            .map_err(|err| {
    ...
    async fn delete_by_doc(&self, doc_id: DocumentId) -> AppResult<()> {
        ...
        sqlx::query(node_text_sql::DELETE_BY_DOC)
        .bind(uuid_to_blob(doc_id.as_uuid()))
        .execute(self.pool.pool())
        .await
        .map_err(|err| {
